...
<div class="container mx-auto px-4 py-6 max-w-2xl">
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="mb-6 border-b pb-4">
            <h1 class="text-3xl font-bold text-gray-800">Edit Class Session</h1>
            <p class="text-gray-600 mt-2">{{ $course->title }}</p>
        </div>

        <form action="/instructor/courses/{{ $course->id }}/class-sessions/{{ $classSession->id }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="title" id="title" placeholder="e.g., Introduction to Laravel"
                       ...
                       value="{{ old('title', $classSession->title) }}" required>
                @error('title')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Describe what will be covered in this session"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $classSession->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="session_date" id="session_date"
                       ...
                       value="{{ old('session_date', $classSession->session_date->format('Y-m-d')) }}" required>
                @error('session_date')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="session_type" id="session_type"
                    ...
                    <option value="online" {{ old('session_type', $classSession->session_type) === 'online' ? 'selected' : '' }}>Online</option>
                    <option value="offline" {{ old('session_type', $classSession->session_type) === 'offline' ? 'selected' : '' }}>Offline (Tatap Muka)</option>
                    <option value="hybrid" {{ old('session_type', $classSession->session_type) === 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                </select>
                @error('session_type')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="meeting_type" id="meeting_type"
                    ...
                    <option value="zoom" {{ old('meeting_type', $classSession->meeting_type) === 'zoom' ? 'selected' : '' }}>Zoom</option>
                    <option value="tatap_muka" {{ old('meeting_type', $classSession->meeting_type) === 'tatap_muka' ? 'selected' : '' }}>Tatap Muka</option>
                </select>
                @error('meeting_type')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div id="zoomLinkDiv" class="hidden">
                ...
                <input type="url" name="zoom_link" id="zoom_link" placeholder="https://zoom.us/j/..."
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                       value="{{ old('zoom_link', $classSession->zoom_link) }}">
                @error('zoom_link')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="locationDiv" class="hidden">
                ...
                <input type="text" name="location" id="location" placeholder="e.g., Building A, Hall 101"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                       value="{{ old('location', $classSession->location) }}">
                @error('location')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="roomNumberDiv" class="hidden">
                ...
                <input type="text" name="room_number" id="room_number" placeholder="e.g., 101"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                       value="{{ old('room_number', $classSession->room_number) }}">
            </div>
            ...
            <div id="offlineNotesDiv" class="hidden">
                ...
                <textarea name="offline_notes" id="offline_notes" rows="2" placeholder="Additional notes for offline session"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('offline_notes', $classSession->offline_notes) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="agenda" id="agenda" rows="3" placeholder="What will be discussed in this session?"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('agenda', $classSession->agenda) }}</textarea>
            </div>
