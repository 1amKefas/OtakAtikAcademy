...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow mb-8">
        ...
        {{-- [FIX] Route name updated to instructor.courses.classes.store --}}
        <form action="{{ route('instructor.courses.classes.store', $course->id) }}" method="POST" class="flex gap-4 items-end">
            @csrf
            <div class="w-1/3">
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-40 bg-white border shadow-lg rounded z-10">
                                    @foreach($classes as $class)
                                        {{-- [FIX] Route name updated to instructor.courses.classes.assign --}}
                                        <form action="{{ route('instructor.courses.classes.assign', [$course->id, $class->id]) }}" method="POST">
                                            @csrf
                ...
                <div class="bg-white border rounded-lg shadow-sm h-fit">
                    <div class="bg-gray-100 p-4 border-b flex justify-between items-start">
                        ...
                        <div class="text-right">
                            ...
                            {{-- [FIX] Route name updated to instructor.courses.classes.destroy --}}
                            <form action="{{ route('instructor.courses.classes.destroy', [$course->id, $class->id]) }}" method="POST" class="mt-2" onsubmit="return confirm('Hapus kelas ini? Siswa akan kembali ke status unassigned.');">
                                @csrf @method('DELETE')
                    ...
                    <div class="p-4 space-y-2 max-h-[300px] overflow-y-auto">
                        @forelse($class->students as $student)
                            <div class="flex justify-between items-center text-sm border-b pb-1 last:border-0">
                                ...
                                {{-- [FIX] Route name updated to instructor.courses.classes.remove-student --}}
                                <form action="{{ route('instructor.courses.classes.remove-student', [$course->id, $student->id]) }}" method="POST">
                                    @csrf @method('DELETE')
